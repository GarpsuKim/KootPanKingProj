...
echo [4/4] EXE 빌드 중 (app-image)...

:: 핵심: --main-jar와 --main-class를 지정하고, --input 폴더를 라이브러리 경로로 활용
set "JP_OPTS=--name %PROJECT% --type app-image --input "%INPUT_DIR%" --dest "%DIST_DIR%" --main-jar %JarFile% --main-class %PROJECT% --app-version 1.0.0 --vendor "KootPanKing""
...
if exist "%ROOT_DIR%\app.ico" (
    set "JP_OPTS=%JP_OPTS% --icon "%ROOT_DIR%\app.ico""
)
